...
function FriendAvatar({ avatarId, username, size = 58 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: 16, display: 'grid', placeItems: 'center', flexShrink: 0, background: 'rgba(83,240,124,.10)', border: '1px solid rgba(83,240,124,.18)', fontWeight: 900 }}>
...
export default function FriendsPage() {
  ...
  useEffect(() => { const timer = setInterval(() => forceTick((value) => value + 1), 1000); return () => clearInterval(timer); }, []);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getPublicProfilesByUids(uids).then((items) => { if (!cancelled) setFriendProfiles(items); }).catch(() => { if (!cancelled) setFriendProfiles({}); });
    return () => { cancelled = true; };
  }, [friends, firebaseUser?.uid]);

  useEffect(() => {
    const uids = friendRequests.map((item) => item.fromUid).filter(Boolean);
    if (!uids.length) { setRequestProfiles({}); return; }
    let cancelled = false;
    getPublicProfilesByUids(uids).then((items) => { if (!cancelled) setRequestProfiles(items); }).catch(() => { if (!cancelled) setRequestProfiles({}); });
    return () => { cancelled = true; };
  }, [friendRequests]);
  ...
  const friendsOutNow = useMemo(() => displayFriends.filter((friend) => friend.activeCheckin).sort((a, b) => (b.activeCheckin?.checkedInAtMillis || 0) - (a.activeCheckin?.checkedInAtMillis || 0)), [displayFriends]);

  const searchResultPresence = useMemo(() => {
    if (!searchResult?.uid) return null;
    const activeCheckin = checkins.find((item) => item.uid === searchResult.uid && item.active) || null;
    if (!activeCheckin) return null;
    return {
      barId: activeCheckin.barId,
      barName: msuBars.find((bar) => bar.id === activeCheckin.barId)?.name || activeCheckin.barId,
    };
  ...
  const handleSearch = async () => {
    ...
    if (!clean) { setSearchResult(null); setSocialFeedback('Enter A Username First.'); return; }
    setSearching(true);
    setSocialFeedback('');
    setSentRequest(null);
    try {
      const user = await findUserByUsername(clean);
      if (!user) { setSearchResult(null); setSocialFeedback('User Not Found.'); return; }
      if (user.uid === firebaseUser.uid) { setSearchResult(null); setSocialFeedback('That’s You.'); return; }
    ...
    <Layout>
      <section className="home-stack" style={{ gap: 18 }}>
        <header style={{ display: 'grid', gap: 14 }}>
          <h1 style={{ margin: 0, fontSize: '2.2rem', fontWeight: 950 }}>Friends</h1>
          ...
          <div style={panelStyle}>
            <div style={rowStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 13, minWidth: 0 }}>
                <FriendAvatar avatarId={searchResult.avatarId} username={searchResult.displayUsername || searchResult.username} size={68} />
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontWeight: 900, fontSize: '1.08rem', overflow: 'hidden', textOverflow: 'ellipsis' }}>@{searchResult.displayUsername || searchResult.username}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 6, color: searchResultPresence ? '#53f07c' : 'rgba(235,255,240,.52)', fontSize: '.9rem' }}>
                    <StatusDot active={Boolean(searchResultPresence)} />
                    {searchResultPresence ? `At ${searchResultPresence.barName}` : 'Not Out Right Now'}
                  </div>
                </div>
              </div>
        ...
        {sentRequest ? (
          <div style={{ ...panelStyle, padding: '28px 22px', textAlign: 'center', gap: 16 }}>
            <div style={{ width: 72, height: 72, borderRadius: '50%', margin: '0 auto', background: '#32b94f', display: 'grid', placeItems: 'center', fontSize: '2.3rem', fontWeight: 900 }}>✓</div>
            <div>
              <h2 style={{ margin: 0, fontSize: '1.45rem' }}>Friend Request Sent!</h2>
              <p style={{ margin: '9px 0 0', color: 'rgba(235,255,240,.62)' }}>@{sentRequest.displayUsername || sentRequest.username} Will Be Notified.</p>
            </div>
            <button className="primary-button" type="button" onClick={() => setSentRequest(null)} style={{ width: '100%' }}>Okay</button>
          </div>
        ) : null}

        {socialFeedback ? <div style={{ padding: '13px 15px', borderRadius: 16, background: 'rgba(83,240,124,.10)', border: '1px solid rgba(83,240,124,.14)', color: '#dfffe8', fontWeight: 800, textAlign: 'center' }}>{socialFeedback}</div> : null}
          ...
          <div style={panelStyle}>
            ...
            {invites.map((invite) => (
              <div key={invite.id} style={rowStyle}>
                <div style={{ minWidth: 0 }}><strong>@{invite.fromUsername || 'friend'}</strong><div style={{ marginTop: 4, color: '#53f07c', fontSize: '.9rem' }}>Invited You To {invite.barName}</div></div>
                <div style={{ display: 'flex', gap: 7 }}><Link to={`/bar/${invite.barId}`} className="ghost-button" style={{ textDecoration: 'none' }}>View</Link><button className="primary-button" onClick={() => dismissInvite(invite.id)}>Got It</button></div>
              </div>
            ))}
          </div>
        ) : null}
            ...
            {displayFriends.length ? displayFriends.map((friend) => {
              ...
              return (
                <div key={friend.id} style={rowStyle}>
                  ...
                  <button className="ghost-button" disabled={disabled} onClick={() => handleInviteFriendToBar(friend)} style={{ opacity: disabled ? .45 : 1, borderColor: disabled ? 'rgba(235,255,240,.12)' : 'rgba(83,240,124,.35)', color: disabled ? 'rgba(235,255,240,.58)' : '#53f07c', maxWidth: 150, whiteSpace: 'normal', lineHeight: 1.2, padding: '10px 12px' }}>
            ...
            {friendRequests.length ? friendRequests.map((requestItem) => {
              ...
              return (
                <div key={requestItem.id} style={rowStyle}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}><FriendAvatar avatarId={senderProfile?.avatarId} username={senderUsername} /><strong style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>@{senderUsername}</strong></div>
                  <div style={{ display: 'flex', gap: 7 }}><button className="primary-button" onClick={() => handleRespondToFriendRequest(requestItem, 'accepted')}>Accept</button><button className="ghost-button" onClick={() => handleRespondToFriendRequest(requestItem, 'declined')}>Decline</button></div>
                </div>
              );
        ...
        <div style={panelStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12 }}><h2 style={{ margin: 0, fontSize: '1.18rem' }}>Who’s Out Right Now</h2><span style={{ color: '#53f07c', fontWeight: 800, fontSize: '.86rem' }}>{friendsOutNow.length} Out</span></div>
          {friendsOutNow.length ? friendsOutNow.map((friend) => (
            <div key={friend.id} style={rowStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}><FriendAvatar avatarId={friend.avatarId} username={friend.username} size={48} /><div style={{ minWidth: 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 7, fontWeight: 900 }}>@{friend.username}<StatusDot active /></div><div style={{ marginTop: 4, color: 'rgba(235,255,240,.60)', fontSize: '.87rem' }}>At {friend.barName} · {timeAgo(friend.activeCheckin?.checkedInAtMillis)}</div></div></div>
